...
public abstract class Resilience4jSampleSseServlet extends HttpServlet {
  ...
  protected Resilience4jSampleSseServlet(
      Observable<String> sampleStream, int pausePollerThreadDelayInMs) {
    this.sampleStream = sampleStream;
    this.pausePollerThreadDelayInMs = pausePollerThreadDelayInMs;
  }

  /**
   * WebSphere won't shutdown a servlet until after a 60 second timeout if there is an instance of
   * the servlet executing a request. Add this method to enable a hook to notify Hystrix to
   * shutdown. You must invoke this method at shutdown, perhaps from some other servlet's destroy()
   * method.
   */
  ...
  /**
   * - maintain an open connection with the client - on initial connection send latest data of each
   * requested event type - subsequently send all changes for each requested event type
   *
  ...
  private void handleRequest(HttpServletRequest request, final HttpServletResponse response)
    ...
    try {
      int maxNumberConnectionsAllowed =
          getMaxNumberConcurrentConnectionsAllowed(); // may change at runtime, so look this up for
      // each request
      if (numberConnections > maxNumberConnectionsAllowed) {
        ...
        // since the sample stream is based on Observable.interval, events will get published on an
        // RxComputation thread
        // since writing to the servlet response is blocking, use the Rx IO thread for the write
        // that occurs in the onNext
        sampleSubscription =
            sampleStream
